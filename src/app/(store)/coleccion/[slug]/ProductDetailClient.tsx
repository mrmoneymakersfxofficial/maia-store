...
export default function ProductDetailClient({ slug: initialSlug }: ProductDetailClientProps) {
  ...
  const switchVariant = useCallback(
    (variant: Product) => {
      ...
      router.replace(`/coleccion/${variant.slug}`, { scroll: false });
    },
    [router],
  );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center pt-20">
        <div className="text-center">
          ...
          <Link href="/coleccion" className="bg-primary text-white px-6 py-3 rounded-full font-semibold hover:bg-turquoise-600 transition-colors">
            Ver Colección
          </Link>
        </div>
      </div>
    ...
    <div ref={pageRef} className="relative pt-16 pb-32 sm:pb-24">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-12 xl:px-16">
        ...
        <nav className="flex items-center gap-1.5 text-xs text-foreground/40 mb-4 pt-2 detail-animate">
          ...
          <ChevronRight className="w-3 h-3" />
          <Link href={`/coleccion?categoria=${displayProduct.category}`} className="hover:text-primary transition-colors capitalize">{displayProduct.categoryLabel}</Link>
          <ChevronRight className="w-3 h-3" />
