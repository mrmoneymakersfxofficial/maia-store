...
interface HomeClientProps {
  ...
  testimonials: SanityTestimonialData[];
  siteSettings: SanitySiteSettingsData | null;
}
...
export default function HomeClient({
  ...
  // Build hero image list from Sanity or fallback
  const heroImages = heroSlides.length > 0
    ? heroSlides
        .filter((s) => s.backgroundImage)
        .map((s) => ({ url: getImageUrl(s.backgroundImage, 1920, 1080) || '', _id: s._id }))
    : [
        { url: '/images/hero-craft.webp', _id: '' },
        { url: '/images/hero-slide-1.webp', _id: '' },
        { url: '/images/hero-slide-2.webp', _id: '' },
      ];
  ...
  const featured = featuredProducts.length > 0 ? featuredProducts.slice(0, 6) : [];
      ...
      <section id="hero" ref={heroRef} className="relative w-full h-screen overflow-hidden scroll-mt-16">
        <div className="absolute inset-0" aria-hidden="true">
          {heroImages.map((slide, i) => (
            <div
              ...
              style={{
                backgroundImage: slide.url ? `url('${slide.url}')` : undefined,
        ...
        <div className="relative z-10 flex flex-col items-center justify-center h-full px-4 text-center">
          ...
          <p
            ...
            style={{ opacity: 0 }}
          >
            {siteSettings?.tagline || 'Cada pieza cuenta una historia. Descubre nuestra coleccion exclusiva de joyeria artesanal, tejida con amor y dedicacion en Peru.'}
          </p>
          ...
          {featured.length > 0 ? (
            ...
            <div className="text-center py-16">
              <p className="text-foreground/40 text-sm">
                No hay productos destacados aun. Agrega productos desde el CMS y marca &quot;Destacado&quot;.
              </p>
            </div>
          )}
              ...
              <div className="mb-5">
                <InfiniteMarquee speed={40} className="py-1">
                  {testimonials.slice(0, 6).map((t, i) => (
                    <SanityTestimonialCard key={`r1-${t._id}-${i}`} testimonial={t} />
                  ))}
                </InfiniteMarquee>
              </div>
              <div>
                <InfiniteMarquee speed={35} direction="right" className="py-1">
                  {[...testimonials].reverse().slice(0, 6).map((t, i) => (
                    <SanityTestimonialCard key={`r2-${t._id}-${i}`} testimonial={t} />
                  ))}
                </InfiniteMarquee>
              </div>
            </>
      ...
      <Link href={`/coleccion/${product.slug}`}>
        <div className="relative aspect-[3/4] rounded-2xl overflow-hidden mb-3 bg-zinc-100">
          ...
          <div className="absolute bottom-3 right-3 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            ...
            <motion.button
              onClick={(e) => { e.preventDefault(); addToCart({ id: product._id, slug: product.slug, name: product.name, price: product.price, image: mainImage || '', categoryLabel, description: product.description || '' }); }}
...
// ─── Testimonial Card (Sanity data) ───────────────────
function SanityTestimonialCard({ testimonial: t }: { testimonial: SanityTestimonialData }) {
  const photoUrl = t.photo ? getImageUrl(t.photo, 80, 80) : null;
  return (
    <div className="flex-shrink-0 w-[320px] sm:w-[380px] p-5 rounded-2xl bg-zinc-50/60 border border-zinc-100/60 backdrop-blur-sm hover:bg-white/80 transition-all duration-500">
      ...
      <p className="text-foreground/60 text-xs leading-relaxed mb-4 line-clamp-3">
        {t.quote
          ? t.quote.map((block: { children?: Array<{ text: string }> }) => block.children?.map((c) => c.text).join('') || '').join(' ')
          : ''}
      </p>
      <div className="flex items-center gap-2.5 pt-3 border-t border-zinc-100">
        {photoUrl ? (
          <img src={photoUrl} alt={t.authorName} className="w-8 h-8 rounded-full object-cover" />
        ) : (
          <div className="w-8 h-8 rounded-full bg-turquoise-100 flex items-center justify-center">
            <span className="text-xs font-bold text-turquoise-700">{t.authorName.charAt(0)}</span>
          </div>
        )}
        <div>
          <p className="text-xs font-semibold text-foreground">{t.authorName}</p>
          <p className="text-[10px] text-foreground/40">{t.company || t.authorRole || ''}</p>
        </div>
      </div>
    </div>
  );
